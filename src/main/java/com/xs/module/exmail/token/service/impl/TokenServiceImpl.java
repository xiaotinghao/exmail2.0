...
public class TokenServiceImpl implements TokenService {
    ...
    public JSONObject getToken(String corpId, String corpSecret) {
        ...
        if (!valid) {
            return (JSONObject) JSONObject.toJSON(Result2.get("INVALID_CREDENTIAL"));
        }

        JSONObject jsonObject = JSONObject.parseObject(Result2.success());

        long millis = System.currentTimeMillis();
        // 待加密字符串
        String str = corpId + SEPARATOR + corpSecret + SEPARATOR + millis;
        // 加密生成accessToken
        String pwd = ConstantsConfig.get("TOKEN_ENCRYPTION_KEY");
        String accessToken = baseDao.encode(str, pwd);
        jsonObject.put(access_token, accessToken);

        // 查询token时效（单位毫秒）
        String timeEfficient = ConstantsConfig.get("TOKEN_TIME_EFFICIENT");
